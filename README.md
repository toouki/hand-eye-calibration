...
已成功将 `compute_in_hand.py` 和 `compute_to_hand.py` 的计算功能集成到 `UI.py` 中，现在可以在同一个界面中完成数据采集和标定计算。

## 新增功能

### 1. 眼在手上标定计算 (Eye-in-Hand)
- **快捷键**: `i` 或 `I`
- **功能**: 计算相机坐标系相对于机械臂末端坐标系的位姿
- **输出**: 旋转矩阵、平移向量、四元数
- **结果文件**: `eye_in_hand_result.txt`

### 2. 眼在手外标定计算 (Eye-to-Hand)
- **快捷键**: `o` 或 `O`
- **功能**: 计算相机坐标系相对于机械臂基座坐标系的位姿
- **输出**: 旋转矩阵、平移向量、四元数
- **结果文件**: `eye_to_hand_result.txt`
...
### 2. 采集数据
1. 程序启动后会自动创建保存目录
2. 使用 `s` 键采集图像和对应的机械臂位姿
3. 确保采集至少5-10组不同位姿的数据以获得好的标定结果

### 3. 进行标定计算
采集完数据后，可以直接在命令行中选择计算模式：

#### 眼在手上标定:
```
请输入操作指令 (s=采集, i=眼在手上计算, o=眼在手外计算, q=退出): i
```

#### 眼在手外标定:
```
请输入操作指令 (s=采集, i=眼在手上计算, o=眼在手外计算, q=退出): o
```
...
eye_hand_data/
└── data20241209XX/
    ├── 0.jpg, 1.jpg, 2.jpg...     # 采集的图像
    ├── poses.txt                   # 机械臂位姿数据
    ├── eye_in_hand_result.txt      # 眼在手上计算结果（如果执行了）
    ├── eye_to_hand_result.txt     # 眼在手外计算结果（如果执行了）
    └── RobotToolPose.csv          # 中间计算文件（自动生成）
```

## 计算结果格式

### 旋转矩阵
3×3的旋转矩阵，表示相机到目标坐标系的旋转关系

### 平移向量
3×1的平移向量，单位为米，表示相机到目标坐标系的平移关系

### 四元数
[x, y, z, w] 格式的四元数，便于在机器人系统中使用

## 注意事项

1. **数据质量**: 确保采集的图像中棋盘格清晰可见，无遮挡
2. **位姿多样性**: 采集不同位置和姿态的数据，避免位姿过于集中
3. **最少数据量**: 建议至少采集5组数据，推荐10-15组
4. **单位一致性**: UI已自动处理单位转换（mm→m, deg→rad）
5. **计算时间**: 根据数据量，计算可能需要几秒到几十秒

## 错误处理

如果出现错误：
1. 检查 `eye_hand_data` 目录是否存在
2. 确认图像文件和 `poses.txt` 文件都存在
3. 检查图像中是否能检测到棋盘格
4. 确认采集的图像数量与位姿数量匹配

## 退出程序

使用 `q` 键安全退出程序，所有资源会自动释放。
